...
static void *find_lazy(uint32_t ncmds, const struct load_command *cmds, uintptr_t slide) {
...
#define CATCH(off, addr) if(sc->fileoff <= (off) && (sc->fileoff + sc->filesize) >= (off)) (addr) = (void *) (sc->vmaddr + slide + (off) - sc->fileoff);
    ...
    for(i = lazy_index; i < lazy_index + lazy_size; i++) {
        ...
        void *newfunc = initHooks((char *) (strs + sym->n_un.n_strx) + 0x1);
        if (newfunc != NULL) {
            *lazy = newfunc;
        } else {
         //   printf("Unhooked symbol %s\n", (char *) (strs + sym->n_un.n_strx) + 0x1);
        }
        lazy++;
    }
